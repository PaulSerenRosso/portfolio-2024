...
<template>
  ...
  <svg overflow="visible" width="100%" height="100%" fill="none"  >
    <defs>
      <filter x="-50%" y="-50%" width="200%" height="200%" overflow="visible" id="filter0_f_301_3" color-interpolation-filters="sRGB" >
        <feGaussianBlur :stdDeviation="this.currentBlurDeviation" result="effect1_foregroundBlur_301_3" />
      </filter>
    </defs>
    <g filter="url(#filter0_f_301_3)">
      <ellipse cx="50%" cy="50%" :rx='this.radius+"%"' :ry='this.radius+"%"' :fill="this.Colors[this.color]" />
    </g>
  </svg>
  </div>
